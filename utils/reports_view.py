...
class ReportsView(ttk.Frame):
    ...
    def create_charts_interface(self, parent):
        ...
        chart_types = [
            ("Rentals by Genre", self.show_genre_chart),
            ("Monthly Trends", self.show_monthly_trend_chart),
            ("Revenue by Genre", self.show_revenue_chart),
            ("Rental Status", self.show_status_chart)
        ]
    ...
    def show_genre_chart(self):
        """Show rentals by genre chart."""
        self.clear_charts()
        
        chart_canvas = self.visualizer.create_rentals_by_genre_chart(self.charts_container)
        if chart_canvas:
    ...
    def show_monthly_trend_chart(self):
        ...
        chart_canvas = self.visualizer.create_monthly_trend_chart(self.charts_container)
        if chart_canvas:
            chart_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            self.current_charts.append(chart_canvas)
    
    def show_revenue_chart(self):
        """Show revenue by genre chart."""
        self.clear_charts()
        
        chart_canvas = self.visualizer.create_revenue_by_genre_chart(self.charts_container)
        if chart_canvas:
            chart_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            self.current_charts.append(chart_canvas)
    
    def show_status_chart(self):
        """Show rental status chart."""
        self.clear_charts()
        
        chart_canvas = self.visualizer.create_rental_status_chart(self.charts_container)
        if chart_canvas:
            chart_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            self.current_charts.append(chart_canvas)
    ...
    def refresh_charts(self):
        ...
        if self.current_charts:
            current_method = getattr(self.visualizer, 
                                   self.current_charts[0].__class__.__name__.replace('CanvasTkAgg', '').lower(), 
                                   None)
            if current_method:
                self.clear_charts()
